from pyecharts import Liquid, Page, Style
...
def create_charts():
    ...
    chart = Liquid("水球图-单数据", **style.init_style)
    chart.add("Liquid", [0.6])
    page.add(chart)

    chart = Liquid("水球图-多数据", **style.init_style)
    chart.add("Liquid", [0.6, 0.5, 0.4, 0.3], is_liquid_outline_show=False)
    page.add(chart)

    chart = Liquid("水球图-圆角", **style.init_style)
    chart.add("Liquid", [0.6, 0.5, 0.4, 0.3], is_liquid_outline_show=False,
              shape='roundRect')
    page.add(chart)

    chart = Liquid("水球图-箭头", **style.init_style)
    chart.add("Liquid", [0.6, 0.5, 0.4, 0.3], is_liquid_outline_show=False,
              shape='arrow')
    page.add(chart)

    chart = Liquid("水球图-动画静止", **style.init_style)
    ...
    charts = Liquid("水球图示例", **chart_init)
