...
def bar_charts():
    ...
    chart = Bar("柱状图-标签旋转", width=WIDTH, height=HEIGHT)
    ...
    chart = Bar("柱状图-瀑布图", width=WIDTH, height=HEIGHT)
    ...
    chart = Bar("柱状图-数据缩放(外)", width=WIDTH, height=HEIGHT)
    chart.add("蒸发量", attr, v1, mark_line=["average"], mark_point=["max", "min"])
    chart.add("降水量", attr, v2, mark_line=["average"], mark_point=["max", "min"],
    ...
    chart = Bar("柱状图-数据缩放(内)", width=1000, height=500)
    chart.add("蒸发量", attr, v1, mark_line=["average"], mark_point=["max", "min"])
    chart.add("降水量", attr, v2, mark_line=["average"], mark_point=["max", "min"],
    ...
    attr = ["{}天".format(i) for i in range(30)]
    v1 = [random.randint(1, 30) for _ in range(30)]
    chart = Bar("柱状图-数据缩放(内)", width=WIDTH, height=HEIGHT)
